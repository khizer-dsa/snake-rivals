...
def runGame():
    ...
    apple = getRandomLocation(snakePos)
    ...
    while True:  # main game loop
        ...
        for event in pygame.event.get():  # event handling loop
            ...
            elif event.type == pygame.KEYDOWN:  # to enable keyboard down keys
...
# ----------------------------------------------------------------------------- #
        ...
        # for boundaries
        if snakePos[HEAD]['x'] == -1 or snakePos[HEAD]['x'] == CELLWIDTH or snakePos[HEAD]['y'] == -1 or snakePos[HEAD]['y'] == CELLHEIGHT or wormPos[HEAD]['x'] == -1 or wormPos[HEAD]['x'] == CELLWIDTH or wormPos[HEAD]['y'] == -1 or wormPos[HEAD]['y'] == CELLHEIGHT:
...
def start_menu():
    ...
    while game:
        ...
        Display.blit(text_start, start_rect)
        Display.blit(text_quit, quit_rect)
        pygame.display.update()
        FPSCLOCK.tick(FPS)
...
def scores(score1, score2):
    ...
    Display.blit(score1Surf, score1Rect)
    Display.blit(score2Surf, score2Rect)
...
def drawSnake(snakePos):
    for coord in snakePos:
        ...
        pygame.draw.rect(Display, DARKGREEN, snakeSegmentRect)
        snakeInnerSegmentRect = pygame.Rect(
            x + 4, y + 4, CELLSIZE - 8, CELLSIZE - 8)
        pygame.draw.rect(Display, GREEN, snakeInnerSegmentRect)
...
def drawWorm(snakePos):
    for coord in snakePos:
        ...
        pygame.draw.rect(Display, RED, wormSegmentRect)
        wormInnerSegmentRect = pygame.Rect(
            x + 4, y + 4, CELLSIZE - 8, CELLSIZE - 8)
        pygame.draw.rect(Display, YELLOW, wormInnerSegmentRect)
...
# Citation
# ----------------------------------------------------------------------------- #
#   Title: How to Create a Pygame Menu Selection
#   Author: Pranjal g
#   Date: 10/16/2019
#   Code version: 1.0
#   Availability: https://www.sourcecodester.com/tutorials/python/11784/python-pygame-simple-main-menu-selection.html 

#   lines 404-452
# ----------------------------------------------------------------------------- #
#   Title: Adding a pause function to our game and Game Over
#   Code version: 1.0
#   Availability: https://pythonprogramming.net/pause-game-pygame/ 
#   lines 475-494
# ----------------------------------------------------------------------------- #